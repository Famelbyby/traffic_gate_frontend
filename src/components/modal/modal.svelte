...
<div class="modal-page">
    <div class="modal-window" bind:this={windowRef}>
        <button type="button" class="modal-close" aria-label="Закрыть модальное окно" onclick={closeModal}>
            <img class="modal-close__img" src={close} alt="Закрыть модальное окно" />
        </button>
        {#if header}
            {@render header()}
        {/if}
