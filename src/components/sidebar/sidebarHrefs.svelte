<script lang="ts">
	...
	$effect(() => {
		...
		currentHrefs.forEach(
			(current) =>
				(current.isActivated = currentPath === current.URLpath),
		);
	});
</script>
...
<div class="sidebar-hrefs">
	{#each currentHrefs as href (href.title)}
		<div class={`sidebar-href sidebar-href_${theme}`}>
			<a
				href={href.URLpath}
				class={href.isActivated ? 'sidebar-href__activated' : ''}
				>{href.title}</a>
		</div>
	{/each}
</div>
